...
class DQN(object):
    def __init__(self, args):  # Define DQN attributes
        self.seed = args.seed
        ...
        # Set max holding number based on dataset
        ...
        # Update n_action from 2 to 3
        self.n_action = 3  # Changed from 2 to 3 for SELL, HOLD, BUY
        
        self.n_state_1 = len(self.tech_indicator_list)
        self.n_state_2 = len(self.tech_indicator_list_trend)
        
        # Update subagent network initialization with 3 actions instead of 2
        ...
        model_list_vol = [
            r"../../result/low_level/ETHUSDT/vol/4/best_model.pkl",
            r"../../result/low_level/ETHUSDT/vol/4/best_model.pkl",
            r"../../result/low_level/ETHUSDT/vol/4/best_model.pkl"
        ]
        
        # Note: You may need to retrain these models with 3 actions
        # or create an adapter layer to map from 2-action to 3-action models
        ...
        # Update hyperagent with 3 actions
        ...
        # You may need to update the episodicmemory to accommodate 3 actions
    ...
    def train(self):
        ...
        for sample in range(self.epoch_number):
            ...
            if return_rate_eval > best_return_rate:
                ...
                best_model = self.hyperagent  # Store the full model, not just state_dict

            epoch_return_rate_train_list = []
            epoch_final_balance_train_list = []
            epoch_required_money_train_list = []
            epoch_reward_sum_train_list = []
            best_model_path = os.path.join(r"../../result/high_level", 
                               '{}'.format(self.dataset), 'best_model.pkl')
            torch.save(best_model.state_dict(), best_model_path)  # Saving model state_dict

        final_result_path = os.path.join(r"../../result", '{}'.format(self.dataset))
    ...
    def test_cluster(self, epoch_path, save_path):
        ...
        ##self.hyperagent.load_state_dict(
           ##torch.load(os.path.join(epoch_path, "trained_model.pkl")))
        self.hyperagent.load_state_dict(
           torch.load(r"C:\Users\KIIT\Downloads\MacroHFT\result\high_level\ETHUSDT\best_model.pkl"))
        self.hyperagent.eval()
        ...
        # Lists to store actions and rewards for the episode
        action_list_episode = []
        reward_list_episode = []

        # Lists to store timestamps and prices for visualization
        timestamps = []
        prices = []
        actions = []

        # Simulate the trading episode
        while not done:
            # Record timestamp and price
            timestamps.append(self.df.iloc[test_env.m]['timestamp'])
            prices.append(self.df.iloc[test_env.m]['close'])
...
# Add price line
        ...
        # Add sell signals (action=0)
        fig.add_trace(go.Scatter(
            x=[timestamps[i] for i in range(len(actions)) if actions[i] == 0],
            y=[prices[i] for i in range(len(actions)) if actions[i] == 0],
            mode='markers',
            name='Sell',
            marker=dict(symbol='triangle-down', color='red', size=10)
        ))

        # Add hold signals (action=1)
        fig.add_trace(go.Scatter(
            x=[timestamps[i] for i in range(len(actions)) if actions[i] == 1],
            y=[prices[i] for i in range(len(actions)) if actions[i] == 1],
            mode='markers',
            name='Hold',
            marker=dict(symbol='circle', color='yellow', size=8)
        ))

        # Add buy signals (action=2)
        fig.add_trace(go.Scatter(
            x=[timestamps[i] for i in range(len(actions)) if actions[i] == 2],
            y=[prices[i] for i in range(len(actions)) if actions[i] == 2],
            mode='markers',
            name='Buy',
        ...
        # Customize layout
        fig.update_layout(
            title='Trading Performance Visualization',
...
if __name__ == "__main__":
    ...
    print(args)
    agent = DQN(args)
    agent.test_cluster(r"C:\Users\KIIT\Downloads\MacroHFT",r"C:\Users\KIIT\Downloads\MacroHFT")
